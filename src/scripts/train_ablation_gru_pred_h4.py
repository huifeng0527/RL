"""Four-step GRU ablation with prediction-conditioned policy.

Single-generation training against h1-h10 plus the stochastic scripted pursuit controller.
Default training order starts with the fourth group:
  4. StrategyGRUPredEndpointExtractor  (GRU + aux loss + predicted endpoint policy input)
  1. MLPOnlyExtractor                  (no history, no aux)
  2. StrategyGRUAuxExtractor           (GRU strategy encoder, no aux loss)
  3. StrategyGRUAuxExtractor           (GRU strategy encoder + four-step aux loss)

Usage:
    python src/scripts/train_ablation_gru_pred_h4.py --steps 8000000
"""
...
from src.utils.feature_extractors import (
    ...
    StrategyGRUPredEndpointExtractor,
)
...
def plot_training_curves(out_dir, all_data):
    ...
    labels = {"4_GRU_Pred_H4": "GRU + Pred H4", "1_MLP": "MLP", "2_GRU_Seq": "GRU", "3_GRU_Aux_H4": "GRU + Aux H4"}
    colors = {"4_GRU_Pred_H4": "#C44E52", "1_MLP": "#4C72B0", "2_GRU_Seq": "#DD8452", "3_GRU_Aux_H4": "#55A868"}
    smooth_window = 500
...
def run(args):
    ...
    out = args.output_dir or f"logs/ablation_gru_pred_h4_h1_h10_{now}"
    ...
    experiments = [
        {
            "name": "4_GRU_Pred_H4",
            "extractor": StrategyGRUPredEndpointExtractor,
            "extractor_kwargs": {
    ...
    print("=" * 60)
    print("GRU H4 Ablation: GRU_Pred_H4 / MLP / GRU_Seq / GRU_Aux_H4")
...
def main():
    p = argparse.ArgumentParser(description="Four-step GRU prediction-conditioned ablation against h1-h10 hand pool.")
    ...
    p.add_argument("--future_horizon", type=int, default=4)
    ...
    p.add_argument(
        "--groups",
        nargs="+",
        choices=["4_GRU_Pred_H4", "1_MLP", "2_GRU_Seq", "3_GRU_Aux_H4"],
        default=["4_GRU_Pred_H4", "1_MLP", "2_GRU_Seq", "3_GRU_Aux_H4"],
    )
    run(p.parse_args())
