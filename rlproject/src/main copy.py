...
class RealWorldRehabEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        target_pose =[center_world[0], center_world[1], 0.115, self.rx_c, self.ry_c, self.rz_c]
    ...
    def step(self, action):
        ...
        target_pose =[target_position_world[0], target_position_world[1], 0.110, self.rx_c, self.ry_c, self.rz_c]
        ...
        print(f"\n[控制] Step {self.ep_steps}, Action: {action}, Action_diff: {action_diff:.4f}, Virtual Target Pixel: {self.virtual_target_pixel}, dt: {safe_dt:.3f}s")
...
if __name__ == '__main__':
    ...
    #         print(f"✅ 紧急微调模型已保存: {save_name}")
        SIM_MODEL_PATH = r"C:\Users\admin\Desktop\huifeng\RL\logs\dual_iterative_0427_1314\iteration_14\robot\robot\best_model.zip" 
        # 2. 你上次微调到一半保存的真机模型 (如果没有就填 None)
        LAST_REAL_MODEL_PATH = get_latest_model("logs/model", prefix="finetuned_ppo_real")
        
        # --- 控制开关 ---
        RESUME_FROM_REAL_MODEL = False # True: 接着上次的真机模型训; False: 拿纯仿真模型开启第一次真机训
