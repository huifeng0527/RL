...
def main():
    ...
    try:
        ...
        while True:
            ...
            current_robot_pixel = np.asarray(real_robot_pixel, dtype=np.float64)
            desired_target_pixel = current_robot_pixel + action_pixel
            desired_before_clip = desired_target_pixel.copy()
            desired_target_pixel[0] = np.clip(desired_target_pixel[0], 50, w_px - 50)
            desired_target_pixel[1] = np.clip(desired_target_pixel[1], 50, h_px - 50)
            target_clipped = not np.allclose(desired_before_clip, desired_target_pixel)

            max_pixel_step = args.max_step * (w_px / W_ENV)
            diff_vec = desired_target_pixel - current_robot_pixel
            dist_pixel = float(np.linalg.norm(diff_vec))
            target_step_limited = dist_pixel > max_pixel_step
            if target_step_limited and dist_pixel > 1e-9:
                target_pixel = current_robot_pixel + (diff_vec / dist_pixel) * max_pixel_step
            else:
                target_pixel = desired_target_pixel.copy()
            ...
            logger.record_step({
                ...
                "virtual_target_x_px": None,
                "virtual_target_y_px": None,
                "desired_target_x_px": float(target_pixel[0]),
                "desired_target_y_px": float(target_pixel[1]),
