# methodology_ieee_v2.19_submission_polish

Complete Markdown review draft revised from `methodology_ieee_v2.18_no_prose_colons.md`. This version applies submission-stage polish by removing repeated roadmap summaries, splitting dense metric and variable-definition sentences, making findings rather than figures the subjects of results prose, and tightening several informal or overly broad phrases.
...
We first compare three robot training protocols to evaluate the effect of opponent diversity. The scripted-only baseline is trained against the stochastic scripted pursuit controller. The single-hand baseline is trained against one learned hand policy. The league policy is trained through iterative opponent-pool expansion. We then evaluate representation design through a network ablation comparing an MLP policy, a GRU policy with temporal interaction history, and a GRU+Aux policy with the auxiliary future-dynamics head.
