# methodology_ieee_v2.56_clarify_h1_table

Stage manuscript draft revised from `methodology_ieee_v2.55_dynamic_deployment_response.md`. This version identifies H1 explicitly in Table II and frames the table as a controlled comparison across representative pursuit conditions.
...
Table II complements the cross-generation analysis in Fig. 3 by comparing the SPC baseline, single policy baseline, and final league policy under the SPC, a fixed learned hand policy (H1), and manual mouse pursuit. The same H1 policy is used to evaluate all three robot policies, providing a controlled condition with a learned pursuit strategy. The manual mouse condition was completed by one healthy operator using the mouse interface to pursue and catch the simulated microrobot as quickly as possible. It introduces human reaction time and voluntary pursuit strategy, but it does not reproduce the biomechanics or perception noise of real hand tracking. It is therefore treated as a human-in-the-loop stress test rather than clinical validation. Higher TIZ and longer episode length indicate more sustained difficulty regulation.

**TABLE II: POLICY COMPARISON ACROSS THREE PURSUIT CONDITIONS**

| Robot policy | Pursuit condition | TIZ | Length |
| --- | --- | ---: | ---: |
| SPC baseline | SPC | 0.31 ± 0.06 | 52 ± 8 |
| SPC baseline | Learned hand (H1) | 0.19 ± 0.04 | 31 ± 5 |
| SPC baseline | Manual mouse | 0.14 ± 0.03 | 33 ± 9 |
| Single policy | SPC | 0.11 ± 0.02 | 25 ± 4 |
| Single policy | Learned hand (H1) | 0.49 ± 0.07 | 70 ± 7 |
| Single policy | Manual mouse | 0.19 ± 0.05 | 40 ± 11 |
| League | SPC | 0.38 ± 0.06 | 67 ± 8 |
| League | Learned hand (H1) | 0.48 ± 0.07 | 64 ± 8 |
| League | Manual mouse | 0.49 ± 0.09 | 74 ± 13 |

Values are means ± approximate 95% confidence intervals estimated across evaluation episodes. SPC and H1 tests used 100 episodes per condition, and manual mouse tests used 20 episodes per robot policy. Learned hand (H1) denotes the fixed first-generation learned hand policy used to evaluate all three robot policies. H1 served as the training opponent for the single policy baseline and was included in the historical hand pool used for league training. TIZ denotes the fraction of the full episode horizon spent in the ZPD, and Length denotes episode length in simulation steps.

The two baselines without league training show complementary forms of specialization. The SPC baseline remains most suitable for the scripted pursuit controller but degrades under H1 and manual pursuit. The single policy baseline performs well against its H1 training opponent but loses robustness when the pursuit behavior changes to scripted or manual control. The league policy gives the most balanced profile across all three conditions, preserving performance under H1 while improving the scripted and manual pursuit cases. This balance is the desired behavior for adaptive difficulty regulation, because hand behavior can change across controllers, sessions, and voluntary strategies.
...
Fig. 5 presents one representative zero-shot physical rollout. The image sequence in Fig. 5(a) and the traces in Fig. 5(b) are aligned in time, linking the observed configurations to changes in hand speed, robot speed, and separation. During steady pursuit, the distance remains within or close to the target ZPD band. When the hand accelerates sharply, the separation briefly falls below the target ZPD band as the hand closes rapidly on the microrobot. Robot speed then increases, and the distance re-enters the target ZPD band within the following few seconds. As the hand subsequently decelerates, the change in relative motion produces another short deviation. Robot speed decreases after the slowdown, and the distance again returns toward the target ZPD band. Similar coupled changes later in the rollout show that the policy modifies microrobot motion as the pursuit dynamics change rather than maintaining a fixed-speed escape response. The final sustained decrease in distance corresponds to catch and episode termination. This representative trial therefore supports the ability of the simulation-trained policy to respond online to abrupt changes in hand motion and restore the intended interaction range after transient disturbances.
