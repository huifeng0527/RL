# methodology_ieee_v2.35_spc_compact_direction

Stage manuscript draft revised from `methodology_ieee_v2.34_spc_piecewise_formula.md`. This version simplifies the SPC equation into a compact stride-times-direction expression and moves the direction-selection details into prose.
...
With probability 1-epsilon, q_t points from the current hand position toward the microrobot. With probability epsilon, q_t is a random unit direction. In our experiments, rho_ep is sampled from U(0.45, 0.70) and epsilon = 0.05. The learned hand policies replace (2) with their policy outputs, but the resulting intended displacement is passed through the same motor-execution model.
