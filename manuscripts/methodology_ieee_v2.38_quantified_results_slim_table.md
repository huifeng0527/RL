# methodology_ieee_v2.38_quantified_results_slim_table

Stage manuscript draft revised from `methodology_ieee_v2.37_prose_consistency_polish.md`. This version adds concrete numerical results to the league and ablation result text and reduces Table II to TIZ and episode length.
...
League training improves both average and difficult-case behavior in the cross-hand evaluation. Across the learned-hand pool, mean TIZ increases from 0.188 for the first-generation robot to 0.282 for the tenth-generation robot. The worst-hand TIZ also improves from 0.138 to 0.204, suggesting reduced specialization to a single pursuit style.

Lower-tail metrics support the same conclusion. CVaR20 TIZ increases from 0.142 to 0.206, so the gain is not driven only by easy hand policies. The final-generation failure decomposition further reveals that the league policy balances too-close and too-far errors while preserving useful ZPD coverage.
...
Temporal interaction history improves the learned controller beyond the MLP baseline. In the final task-quality evaluation, TIZ increases from 0.160 for the MLP policy to 0.197 for the GRU policy and 0.259 for the auxiliary GRU policy. Workspace coverage follows the same trend, increasing from 0.251 to 0.334 and then to 0.412.

The auxiliary prediction examples provide a diagnostic of the learned temporal representation. By predicting short-horizon hand displacement from the recurrent state, the auxiliary head trains the encoder through a self-supervised motion-prediction signal in addition to policy gradients. The auxiliary GRU also reduces mean jerk from 0.165 for the MLP baseline to 0.127, indicating smoother control in the final evaluation.
