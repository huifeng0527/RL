...
The scalar vector and temporal buffer are encoded in separate streams, as shown in Fig.~\ref{fig:architecture}. A multilayer perceptron (MLP) extracts geometric information from the scalar state, while a gated recurrent unit (GRU) processes the displacement sequence~\citelink{b40}. The two stream outputs are concatenated and passed through a fusion MLP to produce the shared representation used by the policy and value heads.
