# methodology_ieee_v2.34_spc_piecewise_formula

Stage manuscript draft revised from `methodology_ieee_v2.33_simple_reward_terms.md`. This version rewrites the SPC intended-displacement equation as a clearer piecewise expression with separate pursuit and exploratory-motion cases.
...
For robot training, the reward is written as the sum of four intuitive terms: distance regulation, action smoothness, workspace-boundary safety, and catch avoidance. Let d_t be the hand-microrobot distance and a_t be the robot action. The robot reward optimized by PPO is summarized as

$$r_t = r_{\mathrm{dist}}(d_t) + r_{\mathrm{smooth}}(a_t) + r_{\mathrm{bound}}(p_{R,t}) + r_{\mathrm{catch}}(d_t). \tag{1}$$

The distance term rewards maintaining the hand-microrobot separation inside the ZPD band and penalizes distances that are too close or too far. The smoothness term discourages abrupt robot motion, the boundary term penalizes leaving the valid workspace, and the catch term penalizes episodes in which the hand reaches the microrobot. Episodes terminate when the microrobot is caught, leaves the valid workspace, or reaches the maximum horizon.
...
The SPC samples a stride rho_ep once per episode and uses it for the whole episode. At each timestep, it usually moves toward the microrobot and occasionally takes a small exploratory step. Its intended displacement is written as

$$u_t^{\mathrm{SPC}} =
\begin{cases}
\rho_{\mathrm{ep}}\dfrac{p_{R,t}-p_{H,t}}{\|p_{R,t}-p_{H,t}\|_2}, & \text{with probability } 1-\epsilon,\\
\rho_{\mathrm{ep}}\xi_t, & \text{with probability } \epsilon,
\end{cases}
\tag{2}$$

where p_R,t and p_H,t are the microrobot and hand positions, xi_t is a random unit direction, rho_ep is sampled from U(0.45, 0.70), and epsilon = 0.05. The learned hand policies replace (2) with their policy outputs, but the resulting intended displacement is passed through the same motor-execution model.
