...
\hypersetup{
  pdftitle={Adaptive Difficulty Regulation for Noncontact Upper-Limb Rehabilitation Using Reinforcement Learning},
  pdfauthor={[First Author Name]; [Second Author Name]; [Corresponding Author Name]},
...
{[First Author Name] \MakeLowercase{\textit{et al.}}: Adaptive Difficulty Regulation for Noncontact Upper-Limb Rehabilitation Using Reinforcement Learning}

\begin{document}

\title{Adaptive Difficulty Regulation for Noncontact Upper-Limb Rehabilitation Using Reinforcement Learning}
\author{[First Author Name], [Second Author Name], and [Corresponding Author Name]
\thanks{Manuscript received [Month Day, Year]; revised [Month Day, Year]. This work was supported by [Funding Agency/Program] under Grant [Grant Number].}
\thanks{[First Author Name] and [Second Author Name] are with [Department], [Institution], [City], [Country] (e-mail: [author@email.edu]).}
\thanks{[Corresponding Author Name] is with [Department], [Institution], [City], [Country] and is the corresponding author (e-mail: [corresponding-author@email.edu]).}}
\maketitle

\begin{abstract}
Adaptive rehabilitation requires task difficulty to remain challenging and attainable as user behavior changes, which fixed trajectories and hand-tuned controllers cannot reliably achieve. This study presents a reinforcement learning framework for a tabletop upper-limb pursuit task in which a participant attempts to catch an unpowered microrobot moved by noncontact magnetic actuation. The controller regulates the hand-microrobot distance within a platform-specific Zone of Proximal Development band. Decoupled randomization separates pursuit strategy from motor execution to generate diverse virtual hand behaviors. A dual-stream encoder combines current geometry with recent relative motion, and an auxiliary prediction task strengthens the representation of short-term hand dynamics. Iterative league training exposes the controller to an expanding pool of pursuit policies through prioritized opponent sampling. Simulation experiments showed that league training improved robustness across pursuit behaviors, while temporal encoding and auxiliary prediction supported smoother difficulty regulation. In a single zero-shot physical rollout, the policy ran through the complete vision-control loop and responded online to changes in hand motion. Together, the simulation results and physical demonstration support the feasibility of learned, interaction-responsive difficulty regulation for noncontact upper-limb rehabilitation.
\end{abstract}

\begin{IEEEkeywords}
adaptive difficulty, domain randomization, league training, physical deployment, reinforcement learning, robot-assisted rehabilitation
\end{IEEEkeywords}
...
Asynchronous multi-stage RL has similarly reduced policy nonstationarity in pursuit--evasion by alternating optimization between frozen pursuer and evader policies \cite{xiaoAMSDRL2024}. Population-based training retains historical opponents to reduce forgetting and overfitting to the latest adversary \cite{vinyalsAlphaStar2019,jaderbergPopulationGames2019}, while AMSPB applies this principle to pursuit--evasion by sampling from a growing population of current and previously trained policies \cite{sanchezRonceroAMSPB2025}. Our formulation follows this broader principle of staged optimization with policy retention, but uses the opponent pool to maintain the interaction within a target ZPD band rather than to maximize capture or escape. It also retains scripted pursuit in the pool and prioritizes learned hands according to their recent competitiveness against the robot policy.

Algorithm~\ref{alg:iterative-league} summarizes this iterative schedule. In each iteration, the robot is first trained against hand policies sampled from the fixed pool. A new hand policy is then trained against the frozen robot, initialized from the previous learned hand when available, and added to the pool before the next robot-training phase. The experiments use $N=10$ league iterations, producing a final pool that contains the SPC and ten learned hand policies. At this scale, policy storage and episode-length-based prioritized opponent sampling were not computational bottlenecks, since each rollout uses a single sampled hand policy. Larger leagues could prune or down-weight redundant policies based on behavioral similarity, persistently low sampling probability, or low exploitability against the current robot.
...
The evaluation examines robustness across pursuit behaviors, the contribution of temporal hand-motion encoding, and zero-shot operation through the physical perception-control loop. Simulation experiments compare training protocols and encoder designs, whereas physical deployment tests whether the learned policy can regulate microrobot motion coherently from measured hand movement without additional training.

All simulation evaluations use the planar workspace and target ZPD band defined in Section~\ref{subsec:problem-formulation}. A timestep is counted as within the zone when the hand-microrobot distance falls inside this band. The same band is retained during physical deployment to preserve consistency between simulation and hardware. It represents a platform-specific evaluation setting rather than a clinically optimal threshold for every user. Further details on the evaluation and deployment protocols are provided in the Supplementary Material.
...
The training-protocol comparison considers an SPC-only baseline, a single-opponent baseline, and the league policy, with the reactive APF controller serving as a nonlearning geometric reference in Table~\ref{tab:policy-comparison}. The learned policies are evaluated across hand generations in Fig.~\ref{fig:league-cross-generation} and under three representative pursuit conditions in Table~\ref{tab:policy-comparison}. The encoder comparison retains the same training setting and changes only the policy representation. The MLP baseline uses instantaneous geometry, the GRU policy adds recent relative hand displacement, and the auxiliary GRU policy further includes the future-dynamics prediction objective.

\subsection{League Training}
\label{subsec:league-robustness}
League training improved both mean performance and the lowest observed performance across learned hand policies. In Fig.~\ref{fig:league-cross-generation}(a), each row represents one robot generation evaluated against all learned hand generations in the pool. Early robot generations perform well over limited portions of the matrix, whereas later generations maintain higher TIZ across a broader set of learned hands. Fig.~\ref{fig:league-cross-generation}(b) quantifies this change. Mean TIZ across the tested hands increased from 0.188 for the first robot generation to 0.282 for the final generation, while the lowest TIZ increased from 0.138 to 0.204. The trajectories were not strictly monotonic as the opponent pool evolved, but the gains in both summary measures indicate reduced dependence on any single learned hand policy.
...
Table~\ref{tab:policy-comparison} extends the cross-generation analysis through a controlled comparison of the reactive APF controller, the SPC-only baseline, the single-opponent baseline, and the final league policy under scripted, learned, and manual pursuit. The learned hand condition uses H1, the policy obtained after the first hand-training phase; the same checkpoint is used to evaluate all learned robot policies and serves as the sole training opponent for the single-opponent baseline. Automated SPC and H1 results are reported as the mean $\pm$ sample standard deviation over 20 independently seeded trials of 1000 episodes each. Manual-mouse results for the three learned robot policies are reported as the mean $\pm$ approximate 95\% confidence interval over 20 episodes completed by one operator, with a maximum horizon of 100 steps. The APF manual-mouse result is reported as a point estimate. All manual-mouse conditions use the same episode seeds, stride randomization, collision threshold, action mapping, and hand-execution settings. The APF controller was evaluated only in simulation. Manual pursuit is treated as a human-in-the-loop stress test rather than clinical validation. Higher TIZ and longer episodes denote more sustained difficulty regulation.
...
Single-opponent baseline & Manual mouse & $0.29 \pm 0.11$ & $55 \pm 16$ \\
League policy & SPC & $0.349 \pm 0.010$ & $59.3 \pm 1.3$ \\
League policy & Learned hand H1 & $0.358 \pm 0.012$ & $49.8 \pm 1.4$ \\
League policy & Manual mouse & $0.52 \pm 0.14$ & $71 \pm 15$ \\
\bottomrule
\end{tabularx}
\end{table*}

The comparison separates specialization from robustness. The SPC-only and single-opponent baselines each degraded outside their training condition, reaching TIZ values of 0.146 against H1 and 0.107 under SPC, respectively. By contrast, the league policy maintained similar performance under SPC and H1, with TIZ values of 0.349 and 0.358. Although the single-opponent baseline remained strongest in its matched H1 condition, the league policy provided the most consistent performance across scripted, learned, and manual pursuit.

\subsection{Network Ablation}
\label{subsec:network-ablation}
Temporal encoding and auxiliary dynamics prediction progressively improved learning under the fixed training protocol. The MLP baseline uses only the instantaneous geometric state, the GRU policy adds a 16-frame history of relative displacement, and the auxiliary GRU includes the dynamics prediction head described in Section~\ref{subsec:dual-stream-encoder}. In Fig.~\ref{fig:network-ablation}(a), both recurrent policies reach higher episode rewards than the MLP baseline, with the auxiliary GRU showing the strongest late-stage performance. The episode-length curves in Fig.~\ref{fig:network-ablation}(b) follow the same overall ordering. These results show that recent relative motion supports more persistent interaction and that the auxiliary objective provides an additional benefit during policy training.

The auxiliary predictions in Fig.~\ref{fig:network-ablation}(c) provide a qualitative check of the learned temporal representation. The predicted short-horizon displacement follows the direction and curvature of the observed future hand motion rather than merely reconstructing the current position. Fig.~\ref{fig:network-ablation}(d) quantifies the corresponding changes in final control performance. TIZ increased from 0.161 with the MLP baseline to 0.197 with the GRU and to 0.259 with the auxiliary GRU. Workspace coverage followed the same ordering, while mean jerk decreased with the auxiliary objective \cite{balasubramanianSmoothness2015}. The control metrics provide the primary evidence for the auxiliary head, and the prediction examples remain a representation-level consistency check.
...
The simulation-trained policy operated through the physical perception-control loop without additional training. During pilot deployment, human pursuit was tracked across the acrylic workspace while the robotic arm moved the magnetic actuator beneath the surface. Camera observations and robot commands were processed online throughout the recorded rollouts, allowing the fixed policy to regulate microrobot motion under continuous feedback.

The deployment retained the observation structure used in simulation. Calibrated camera measurements provided the hand and microrobot positions, while workspace-boundary features, the previous robot command, and the recent history of relative hand displacement completed the policy input in \eqref{eq:observation} and \eqref{eq:scalar-state}. Robot state measurements supported command execution and system monitoring but were excluded from the policy observation. The perception pipeline combined YOLO-based microrobot detection \cite{redmonYOLO2016}, MediaPipe Hands tracking \cite{zhangMediaPipeHands2020}, camera calibration, and a planar homography from image coordinates to the workspace \cite{zhangCameraCalibration2000}. Brief interruptions in hand tracking were bridged by extrapolating motion from the latest velocity estimate. Workspace limits, bounded command increments, automatic stopping after capture, controlled shutdown behavior, and a hardware emergency stop constrained physical operation.

Fig.~\ref{fig:physical-deployment} presents a representative zero-shot rollout with image frames and aligned motion traces. During steady pursuit, the hand-microrobot distance remains near the target ZPD band. A sharp increase in hand speed briefly reduces the separation below the target range. The policy then increases microrobot speed, and the distance moves back toward the band over the following seconds. When the hand slows, microrobot speed decreases and the separation again approaches the target range. Similar coupled responses throughout the rollout indicate that the policy adjusts microrobot motion to changes in pursuit dynamics rather than applying a fixed-speed escape command. The final decrease in distance corresponds to capture and episode termination. Overall, the representative rollout supports online responsiveness to abrupt changes in hand motion and recovery toward the intended interaction range after transient deviations.
...
At the same hand-microrobot distance, the appropriate robot action can depend on the preceding motion. The hand may be accelerating toward the target, recovering after a failed approach, changing direction, or responding after a delay. These cases are difficult to distinguish from the instantaneous geometric state alone. Encoding recent relative displacement gives the policy access to motion direction and short-term interaction trends, consistent with the performance differences in Fig.~\ref{fig:network-ablation}. The auxiliary objective may strengthen this temporal representation by encouraging the shared encoder to retain information useful for short-horizon hand-motion prediction. Prediction itself is not the control objective. It provides an additional training signal for motion features that support difficulty regulation, while policy performance remains the primary criterion \cite{choRNNEncoderDecoder2014,jaderbergAuxiliaryTasks2017,schwarzerSPR2021,pathakCuriosity2017}.

Variability also arises from differences among pursuit behaviors. The scripted controller mainly produces direct pursuit, whereas learned hand policies can develop interception-oriented responses. Separating strategy generation from motor execution expands this variation through inertia, motion-change limits, observation noise, and delay. League training widens the strategy distribution further by adding new hand policies while retaining earlier ones in the pool. The nonmonotonic cross-generation trajectories in Fig.~\ref{fig:league-cross-generation} reflect shifts in this distribution as robot and hand optimization alternate. The condition-specific comparisons in Table~\ref{tab:policy-comparison} clarify the resulting trade-off. Training against a single learned hand favors the matched opponent but degrades under scripted pursuit, whereas league training remains more balanced across scripted, learned, and manual pursuit. The main benefit of league training is therefore reduced specialization rather than the highest score against any single behavior \cite{lanctotUnifiedMARL2017,vinyalsAlphaStar2019,jaderbergPopulationGames2019,huangOpponentAwareLeague2023}.
