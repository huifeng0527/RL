# methodology_ieee_v2.32_problem_reward_prose_revision

Stage manuscript draft revised from `methodology_ieee_v2.31_reward_table_physical_revision.md`. This version simplifies the reward formulation, improves the logic of the pursuit-evasion problem statement, and strengthens the hardware-platform paragraph while preserving the v2.31 terminology and physical-deployment updates.
...
For robot training, the reward combines the ZPD distance objective with safety and smoothness terms. Let d_t be the hand-microrobot distance, [d_min,d_max] be the target ZPD band, d_c be the catch threshold, Omega be the valid workspace, and a_t be the robot action. The robot reward optimized by PPO is summarized as

$$r_t = r_{\mathrm{ZPD}}(d_t) - w_c\mathbb{I}(d_t<d_c) - w_b\mathbb{I}(p_{R,t}\notin\Omega) - w_a\|a_t-a_{t-1}\|_2^2. \tag{1}$$

The term r_ZPD(d_t) rewards distances inside the ZPD band and penalizes interactions that become too close or too far. The remaining terms penalize early catch events, workspace violations, and abrupt robot motion. Episodes terminate when the microrobot is caught, leaves the valid workspace, or reaches the maximum horizon. This objective trains the robot to sustain an informative ZPD-inspired interaction rather than simply maximizing escape distance.
