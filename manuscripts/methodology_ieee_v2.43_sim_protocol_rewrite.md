# methodology_ieee_v2.43_sim_protocol_rewrite

Stage manuscript draft revised from `methodology_ieee_v2.42_ablation_section_rewrite.md`. This version rewrites the simulation protocol and metrics section to clarify the evaluation design and align the metric definitions with the result sections.
...
The simulation study separates two sources of performance gain in the proposed framework. The league analysis tests whether expanding the hand-policy pool improves robustness across pursuit behaviors. The ablation analysis tests whether temporal hand-motion encoding and the auxiliary dynamics objective improve the learned robot representation. Both analyses use the pursuit-evasion environment described in Section II, with the robot controlling the microrobot and the hand motion generated by the behavior models described in Section III.

All simulation evaluations used the same planar workspace and ZPD definition. The target ZPD interval was fixed to 3.5-5.5 workspace units, and a timestep was counted as in-zone when the hand-microrobot distance fell within this interval. The simulator and the physical workspace share the same 15 x 10 planar coordinate frame, so one workspace unit corresponds to 1 cm after calibration. This convention maps the simulated ZPD interval directly to 3.5-5.5 cm in the physical rollout analysis.

Time-In-Zone (TIZ) is the primary metric. It measures the fraction of the full episode horizon for which the hand-microrobot distance remains inside the ZPD interval. Episode length is reported in simulation steps and measures how long the interaction remains active before catch, workspace exit, or the maximum horizon. For the league analysis, cross-hand robustness is summarized by mean TIZ across learned hand policies and by worst-hand TIZ. For the network ablation, TIZ is complemented by workspace coverage and mean jerk, which respectively measure task coverage and motion smoothness in the final policy evaluation.

The training-protocol comparison uses an SPC-only robot trained against the stochastic scripted pursuit controller, a single-hand robot trained against one learned hand policy, and a league-trained robot trained through iterative hand-pool expansion. These policies are evaluated through cross-generation learned-hand tests and through the scripted, learned, and manual hand interfaces used in Table II. The representation comparison keeps the training setting fixed while changing the policy encoder. The MLP baseline uses instantaneous geometry, the GRU policy adds recent relative hand displacement, and the auxiliary GRU policy adds the future-dynamics prediction objective.

### B. League Training and Cross-Hand Robustness

The league evaluation asks whether an expanding hand-policy pool improves robustness across pursuit behaviors, rather than only improving performance against the most recent hand policy. After each robot generation is trained, it is evaluated against all learned hand generations in the pool. This cross-generation test separates generalization across hand behaviors from progress against a single training opponent.

The cross-generation matrix in Fig. 3(a) fixes one robot generation per row and one learned hand generation per column. Early robot generations perform well only for limited parts of the hand pool, indicating sensitivity to the particular pursuit behavior used for training. Later generations maintain stronger TIZ over a broader set of learned hands, which suggests that league training reduces narrow specialization.

The mean and worst-hand curves in Fig. 3(b) summarize average and difficult-case robustness across the learned-hand pool. The mean curve reflects typical performance, while the worst-hand curve tracks the most difficult learned hand for each robot generation. Both curves improve from early to final generations despite local fluctuations, showing that the final policy improves the typical interaction while also reducing the weakest-case failure.

**Fig. 3. Cross-hand validation of league-trained robot policies under the ZPD 3.5-5.5 simulation setting. (a) TIZ matrix obtained by evaluating each robot generation against each learned hand generation. Rows correspond to robot generations, and columns correspond to learned hand generations. (b) Mean and worst-hand TIZ across learned hand test policies for each robot generation.**

Table II evaluates whether this robustness extends beyond the learned-hand pool used in Fig. 3. The final SPC-only, single-hand, and league-trained robot policies are tested against the stochastic scripted pursuit controller, a learned hand policy, and manual mouse pursuit. Manual mouse pursuit introduces human reaction time and voluntary pursuit strategy, so it is treated as a human-in-the-loop stress test rather than another automated hand policy. Higher TIZ and longer episode length indicate more sustained difficulty regulation.
...
The two non-league baselines show complementary forms of specialization. The SPC-only robot remains most suitable for the scripted pursuit controller but degrades under learned and manual pursuit. The single-hand robot performs well against the learned hand policy but loses robustness when the pursuit behavior changes to scripted or manual control. The league-trained robot gives the most balanced profile across all three test sets, preserving learned-hand performance while improving the scripted and manual-pursuit cases. This balance is the desired behavior for adaptive difficulty regulation, because hand behavior can change across controllers, sessions, and voluntary strategies.

### C. Network Ablation and Auxiliary Dynamics Analysis

The league results evaluate the diversity of hand behaviors used during training. The network ablation then tests whether the robot policy can use temporal hand-motion information once that diversity is present. The MLP baseline uses the instantaneous geometric state only. The GRU policy adds the 16-frame relative-displacement history, and the auxiliary GRU further adds the future-dynamics prediction head described in Section III.B.

The training curves in Fig. 4(a) and Fig. 4(b) show that temporal history improves optimization and interaction persistence. The GRU policies reach higher episode reward and longer episode length than the MLP baseline, indicating that current geometry alone is insufficient for stable difficulty regulation when hand motion changes over time. The auxiliary GRU gives the strongest training curve, suggesting that the prediction loss helps the recurrent stream form a more useful motion representation.

The auxiliary prediction examples in Fig. 4(c) provide a qualitative check on this representation. The predicted short-horizon displacement generally follows the direction and curvature of the observed future hand motion, showing that the auxiliary head learns local motion dynamics rather than only reconstructing the current position.

The final normalized metrics in Fig. 4(d) summarize the control effect of these representation changes. Recurrent history improves in-zone behavior and workspace coverage relative to the MLP baseline. Adding the auxiliary prediction objective further improves these task-quality metrics while reducing mean jerk. Together, the ablation indicates that both temporal hand history and auxiliary dynamics prediction contribute to smoother and more reliable difficulty regulation.

**Fig. 4. Network ablation for temporal encoding and auxiliary dynamics prediction. (a) Smoothed episode reward during training for the MLP, GRU, and auxiliary GRU policies. (b) Smoothed episode length during training for the same policies. (c) Representative auxiliary-head predictions of short-horizon hand displacement compared with observed future motion. (d) Final task-quality metrics normalized to the MLP baseline; higher values are better for in-zone time and workspace coverage, whereas lower values are better for mean jerk.**
