# methodology_ieee_v2.24_mouse_hand_results

Complete Markdown review draft revised from `methodology_ieee_v2.23_final_natural_flow.md`. This version adds the mouse-controlled hand evaluation results and analysis while preserving the existing automated hand-policy comparison.
...
The same physical layout supports both safety and reproducibility. The acrylic interface reduces risks associated with direct-contact devices, including impact, excessive traction, and local compression. At the same time, the hardware requires only a collaborative robot, a camera, and a permanent magnet; it does not rely on wearable components, force sensors, onboard batteries, or custom actuators. Passive magnetic actuation keeps the patient-side device lightweight and unpowered, which simplifies setup while limiting the consequences of controller errors during development.
...
A generalizable robot policy must account for variation in both patient strategy and motor execution. CMD-DR separates these two sources of variability. A hand policy sampled from the opponent pool determines the intended displacement at each timestep. The pool contains scripted pursuit controllers and trained RL controllers that can intercept or anticipate the microrobot. The intended displacement is then transformed by motor and sensing constraints. The raw displacement is passed through a first-order low-pass filter to model muscle inertia, as shown in (1).

x_t = alpha x_{t-1} + (1 - alpha) u_t,   alpha in (0, 1).     (1)

where u_t denotes the raw desired displacement and x_t denotes the filtered displacement. Acceleration clipping limits abrupt changes in motion, as shown in (2).
...
CMD-DR broadens the range of patient behaviors encountered during training, but the resulting variability must still be inferred from observation. The current hand position alone cannot distinguish deliberate pursuit from sensing noise, motor delay, tremor-like perturbations, or slow response. We therefore encode the observation with two streams, one for the current geometric context and one for recent hand-motion history.
...
The dual-stream architecture makes motion history available to the policy, but it does not by itself guarantee that the recurrent representation captures patient dynamics. The ZPD-based reward provides a control signal, but it does not directly supervise how the encoder should represent patient motion. To provide a denser learning signal, we attach a future-dynamics head to the fused representation.

The head predicts the patient's relative displacement over an eight-step horizon and estimates near-catch risk. Because future displacements are already available from the rollout, the auxiliary targets require no additional annotation. The trajectory loss is given in (5).

L_traj = E[||D_hat_{t+1:t+H} - D_{t+1:t+H}||_2^2],   H = 8.     (5)

where D_hat_{t+1:t+H} is the predicted future displacement sequence and D_{t+1:t+H} is the observed future displacement sequence. The full training objective combines the PPO loss with the auxiliary trajectory and risk-prediction losses in (6).

L_total = L_PPO + lambda_traj L_traj + lambda_risk L_risk.     (6)

where lambda_traj and lambda_risk control the auxiliary loss weights. Because the future-dynamics head shares the encoder with the policy, its gradients also update the spatial and temporal streams. These auxiliary gradients encourage the encoder to capture patient inertia, delay, and interaction dynamics, helping the policy anticipate near-future hand motion from recent history.

### C. League Training

A useful motion representation is not enough if training exposes the robot to a narrow or unstable set of hand behaviors. Training both the robot and hand policies simultaneously is a straightforward baseline, but it creates a non-stationary learning problem. The return of each agent depends on the policy of the other agent. When both policies are updated together, each update changes the environment faced by the other policy, violating the stationarity assumption of Proximal Policy Optimization (PPO). In practice, the interaction can enter a cycle in which the robot exploits the current hand policy, the hand adapts, and the robot loses strategies that were useful against earlier hands. We avoid this issue by freezing one side of the interaction while training the other. During robot training, the hand policies are fixed and treated as part of the environment. During hand training, the robot policy is fixed.
...
Policy comparison across automated hand-controller types provides a stricter test of specialization. The final scripted-only, single-hand, and league-trained robot policies are evaluated against both the stochastic scripted pursuit controller and a learned hand policy. Mouse-controlled human-in-the-loop testing is reported separately because it introduces human reaction time and voluntary strategy rather than another automated controller.
...
Training against a narrow hand model leads to complementary failure modes across the two test conditions. The scripted-only robot remains competitive on the scripted controller but is caught frequently by the learned hand policy, indicating that rule-based pursuit during training does not cover learned strategic behavior. The single-hand robot shows the reverse failure; it performs well against the learned hand policy but degrades sharply on the scripted controller. The league-trained robot is not the best on every scalar metric, but it avoids the severe cross-condition collapse seen in the baselines. For adaptive rehabilitation, this balanced performance is more important than optimizing a single test condition, because patient behavior can shift across sessions and even within a single episode.

Mouse-controlled pursuit provides a separate human-in-the-loop stress test. Unlike the scripted and learned hand policies, the mouse-controlled hand includes human reaction delay and voluntary pursuit strategy, so it is not treated as another automated opponent in Table II.

**TABLE III: MOUSE-CONTROLLED HAND EVALUATION**

| Robot policy | Mouse-hand TIS | Mouse-hand ZPD | Mouse-hand length | Mouse-hand catch |
| --- | --- | --- | --- | --- |
| Scripted-only | 0.14 | 0.50 | 33 | 100% |
| Single-hand | 0.19 | 0.48 | 40 | 95% |
| League | 0.49 | 0.66 | 74 | 60% |

Under mouse-controlled pursuit, the two non-league baselines are caught in nearly all episodes and spend little time in the ZPD. The league policy produces substantially longer interactions with higher TIS and ZPD coverage, while reducing the catch rate from near saturation to 60%. The result is consistent with the automated-controller comparison. Training against an expanding opponent pool improves robustness to hand behavior that is not represented by a single scripted or learned controller. The mouse-controlled condition remains challenging, so this result should be interpreted as additional human-in-the-loop robustness evidence rather than a full clinical validation.
