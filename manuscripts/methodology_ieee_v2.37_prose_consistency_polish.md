# methodology_ieee_v2.37_prose_consistency_polish

Stage manuscript draft revised from `methodology_ieee_v2.36_notation_unit_clarity.md`. This version strengthens the active-pursuit motivation, clarifies self-supervised auxiliary learning and PFSP curriculum logic, standardizes manual mouse pursuit terminology, defines catch rate more precisely, and condenses selected result prose.
...
League training improves both average and difficult-case behavior in the cross-hand evaluation. Later robot generations improve across a broader set of learned hand policies rather than only against the final hand policy, suggesting reduced specialization to a single pursuit style.

Lower-tail metrics support the same conclusion. Mean TIZ, worst-hand TIZ, and CVaR20 improve together, suggesting that the gain is not driven only by easy hand policies. The final-generation failure decomposition further reveals that the league policy balances too-close and too-far errors while preserving useful ZPD coverage.
...
Policy comparison across three hand test sets provides a stricter test of specialization. The final SPC-only, single-hand, and league-trained robot policies are evaluated against the SPC, a learned hand policy, and manual mouse pursuit, as summarized in Table II. Manual mouse pursuit introduces human reaction time and voluntary pursuit strategy, so it is interpreted as a human-in-the-loop stress test rather than another automated hand policy. Higher TIZ, ZPD coverage, and episode length are preferred, whereas lower catch rate is preferred.
...
Temporal interaction history improves the learned controller beyond the MLP baseline. Both recurrent policies produce longer and more stable training episodes, while auxiliary future-dynamics supervision strengthens the final task-quality profile with higher TIZ, broader workspace coverage, and lower jerk.

The auxiliary prediction examples provide a diagnostic of the learned temporal representation. By predicting short-horizon hand displacement from the recurrent state, the auxiliary head trains the encoder through a self-supervised motion-prediction signal in addition to policy gradients. These predictions serve both as an interpretable representation check and as an additional training signal for the control policy.
