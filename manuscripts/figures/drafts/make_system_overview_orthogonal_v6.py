...
px, py = rx, ty
box(px + 0.035, py + 0.055, 0.335, 0.240, fc="#FFFFFF", ec=C["border"], lw=0.9, r=0.018)
txt(px + 0.095, py + 0.266, "Input layer", size=6.6, weight="bold")
txt(px + 0.198, py + 0.266, "Hidden layers", size=6.6, weight="bold")
txt(px + 0.315, py + 0.266, "Output", size=6.6, weight="bold")

# Input variables grouped as state and history, without exposing the detailed network architecture.
box(px + 0.050, py + 0.183, 0.040, 0.070, fc=C["orange_l"], ec=C["orange"], lw=0.8, r=0.010)
box(px + 0.050, py + 0.075, 0.040, 0.070, fc=C["orange_l"], ec=C["orange"], lw=0.8, r=0.010)
txt(px + 0.037, py + 0.218, "task\nstate", size=6.0, ha="right")
txt(px + 0.037, py + 0.110, "motion\nhistory", size=6.0, ha="right")
input_nodes = [(px + 0.070, py + yy) for yy in [0.235, 0.210, 0.125, 0.100]]
for xnode, ynode in input_nodes:
    ax.add_patch(Circle((xnode, ynode), 0.011, facecolor=C["orange_l"], edgecolor=C["orange"], lw=0.7, zorder=8))
for yy in [0.180, 0.165, 0.150]:
    ax.add_patch(Circle((px + 0.070, py + yy), 0.0035, facecolor=C["black"], edgecolor="none", zorder=8))

hidden1 = [(px + 0.165, py + yy) for yy in [0.245, 0.215, 0.185, 0.125, 0.095, 0.065]]
hidden2 = [(px + 0.235, py + yy) for yy in [0.245, 0.215, 0.185, 0.125, 0.095, 0.065]]
for nodes in [hidden1, hidden2]:
    for xnode, ynode in nodes:
        ax.add_patch(Circle((xnode, ynode), 0.012, facecolor=C["blue_l"], edgecolor=C["blue"], lw=0.7, zorder=8))
for yy in [0.158, 0.142]:
    ax.add_patch(Circle((px + 0.165, py + yy), 0.0035, facecolor=C["black"], edgecolor="none", zorder=8))
    ax.add_patch(Circle((px + 0.235, py + yy), 0.0035, facecolor=C["black"], edgecolor="none", zorder=8))
output = (px + 0.320, py + 0.155)
ax.add_patch(Circle(output, 0.014, facecolor=C["green_l"], edgecolor=C["green"], lw=0.8, zorder=8))
txt(px + 0.348, py + 0.155, "robot\naction", size=6.2, ha="left")

for x1, y1 in input_nodes:
    for x2, y2 in hidden1:
        ax.add_line(Line2D([x1 + 0.011, x2 - 0.012], [y1, y2], color="#C9D3DF", lw=0.30, zorder=6))
for x1, y1 in hidden1:
    for x2, y2 in hidden2:
        ax.add_line(Line2D([x1 + 0.012, x2 - 0.012], [y1, y2], color="#C9D3DF", lw=0.30, zorder=6))
for x1, y1 in hidden2:
    ax.add_line(Line2D([x1 + 0.012, output[0] - 0.014], [y1, output[1]], color="#C9D3DF", lw=0.30, zorder=6))

box(px + 0.140, py + 0.020, 0.120, 0.030, "PPO update", fc=C["green_l"], ec=C["green"], size=6.4)
txt(px + 0.028, py + 0.035, r"reward $r_t$", size=6.1, color=C["purple"], ha="left")
arrow(px + 0.085, py + 0.035, px + 0.140, py + 0.035, color=C["purple"], lw=0.8, ms=6)
arrow(px + 0.200, py + 0.050, px + 0.200, py + 0.068, color=C["green"], lw=0.8, ms=6)
...
for path in [
    OUT_DIR / "fig_overview_system_framework_orthogonal_v6.png",
    DRAFT_DIR / "fig_overview_system_framework_orthogonal_v6_draft.png",
]:
    fig.savefig(path, dpi=450, bbox_inches="tight", pad_inches=0.035, facecolor="white")
fig.savefig(OUT_DIR / "fig_overview_system_framework_orthogonal_v6.pdf", bbox_inches="tight", pad_inches=0.035, facecolor="white")
fig.savefig(OUT_DIR / "fig_overview_system_framework_orthogonal_v6.svg", bbox_inches="tight", pad_inches=0.035, facecolor="white")
plt.close(fig)

print((OUT_DIR / "fig_overview_system_framework_orthogonal_v6.png").as_posix())
print((OUT_DIR / "fig_overview_system_framework_orthogonal_v6.pdf").as_posix())
print((OUT_DIR / "fig_overview_system_framework_orthogonal_v6.svg").as_posix())
print((DRAFT_DIR / "fig_overview_system_framework_orthogonal_v6_draft.png").as_posix())
