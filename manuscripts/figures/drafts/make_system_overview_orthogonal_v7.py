...
px, py = rx, ty
network_x, network_y, network_w, network_h = px + 0.045, py + 0.085, 0.330, 0.215
box(network_x, network_y, network_w, network_h, fc="#FFFFFF", ec=C["border"], lw=0.9, r=0.018)
txt(px + 0.092, py + 0.273, "Input", size=6.7, weight="bold")
txt(px + 0.210, py + 0.273, "Policy network", size=6.7, weight="bold")
txt(px + 0.330, py + 0.273, "Output", size=6.7, weight="bold")

# Left input column: two grouped inputs, drawn large enough to remain readable.
box(px + 0.058, py + 0.196, 0.055, 0.055, fc=C["orange_l"], ec=C["orange"], lw=0.8, r=0.010)
box(px + 0.058, py + 0.112, 0.055, 0.055, fc=C["orange_l"], ec=C["orange"], lw=0.8, r=0.010)
txt(px + 0.045, py + 0.224, "task\nstate", size=6.2, ha="right")
txt(px + 0.045, py + 0.140, "motion\nhistory", size=6.2, ha="right")
input_nodes = [(px + 0.086, py + yy) for yy in [0.235, 0.214, 0.151, 0.130]]
for xnode, ynode in input_nodes:
    ax.add_patch(Circle((xnode, ynode), 0.0090, facecolor=C["orange_l"], edgecolor=C["orange"], lw=0.7, zorder=8))
for yy in [0.190, 0.181, 0.172]:
    ax.add_patch(Circle((px + 0.086, py + yy), 0.0032, facecolor=C["black"], edgecolor="none", zorder=8))

# Center: sparse generic neural network, with fewer edges to avoid visual clutter.
hidden1 = [(px + 0.178, py + yy) for yy in [0.238, 0.205, 0.140, 0.107]]
hidden2 = [(px + 0.245, py + yy) for yy in [0.238, 0.205, 0.140, 0.107]]
for nodes in [hidden1, hidden2]:
    for xnode, ynode in nodes:
        ax.add_patch(Circle((xnode, ynode), 0.0105, facecolor=C["blue_l"], edgecolor=C["blue"], lw=0.7, zorder=8))
for yy in [0.174, 0.160]:
    ax.add_patch(Circle((px + 0.178, py + yy), 0.0032, facecolor=C["black"], edgecolor="none", zorder=8))
    ax.add_patch(Circle((px + 0.245, py + yy), 0.0032, facecolor=C["black"], edgecolor="none", zorder=8))
output = (px + 0.332, py + 0.172)
ax.add_patch(Circle(output, 0.014, facecolor=C["green_l"], edgecolor=C["green"], lw=0.8, zorder=8))
txt(px + 0.333, py + 0.130, "robot action\n$a_t$", size=6.3)

for x1, y1 in input_nodes:
    for x2, y2 in hidden1:
        if abs(y1 - y2) < 0.090:
            ax.add_line(Line2D([x1 + 0.009, x2 - 0.010], [y1, y2], color="#C9D3DF", lw=0.33, zorder=6))
for x1, y1 in hidden1:
    for x2, y2 in hidden2:
        if abs(y1 - y2) < 0.070:
            ax.add_line(Line2D([x1 + 0.010, x2 - 0.010], [y1, y2], color="#C9D3DF", lw=0.33, zorder=6))
for x1, y1 in hidden2:
    ax.add_line(Line2D([x1 + 0.010, output[0] - 0.014], [y1, output[1]], color="#C9D3DF", lw=0.33, zorder=6))

# PPO update is separated from the network to avoid squeezing the nodes.
box(px + 0.125, py + 0.035, 0.170, 0.034, "PPO update from reward $r_t$", fc=C["green_l"], ec=C["green"], size=6.5)
arrow(px + 0.210, py + 0.069, px + 0.210, py + 0.085, color=C["green"], lw=0.8, ms=6)
...
for path in [
    OUT_DIR / "fig_overview_system_framework_orthogonal_v7.png",
    DRAFT_DIR / "fig_overview_system_framework_orthogonal_v7_draft.png",
]:
    fig.savefig(path, dpi=450, bbox_inches="tight", pad_inches=0.035, facecolor="white")
fig.savefig(OUT_DIR / "fig_overview_system_framework_orthogonal_v7.pdf", bbox_inches="tight", pad_inches=0.035, facecolor="white")
fig.savefig(OUT_DIR / "fig_overview_system_framework_orthogonal_v7.svg", bbox_inches="tight", pad_inches=0.035, facecolor="white")
plt.close(fig)

print((OUT_DIR / "fig_overview_system_framework_orthogonal_v7.png").as_posix())
print((OUT_DIR / "fig_overview_system_framework_orthogonal_v7.pdf").as_posix())
print((OUT_DIR / "fig_overview_system_framework_orthogonal_v7.svg").as_posix())
print((DRAFT_DIR / "fig_overview_system_framework_orthogonal_v7_draft.png").as_posix())
