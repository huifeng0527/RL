...
src = Path('manuscripts/methodology_ieee_v2.9_restructured_experiments.docx')
out = Path('manuscripts/methodology_ieee_v2.10_experiment_flow.docx')
...
add_para('The scripted-hand curve serves as a consistency check, but the learned-hand matrix is more diagnostic because it exposes strategy-dependent failures. Together, these results support the use of league-trained virtual patients as a stress-test distribution for adaptive rehabilitation policies.')
...
add_para('Robustness should also appear when the hand controller changes type, not only when the opponent index changes within the learned pool. We therefore compare the final policies against two automated hand-control mechanisms: a noisy scripted pursuit controller and a learned hand agent. The scripted controller represents rule-based pursuit, whereas the learned hand agent can exploit interaction patterns acquired through RL training.')
...
for row in [
    ...
    for cell, value in zip(cells, row):
        cell.text = value
add_para('Table II reports TIS, ZPD coverage, episode length, and catch rate. The scripted-only baseline remains relatively effective on the scripted hand but is frequently caught by the learned agent. The single-agent baseline shows the opposite pattern, performing well against the learned hand while degrading on the scripted controller. These complementary failures indicate brittle specialization to the training opponent type. In contrast, the league-trained robot avoids the strongest form of either failure mode and provides the most balanced behavior across the two automated test conditions. Mouse-controlled human-in-the-loop evaluation is treated as a separate validation stage.')
